...
namespace VolumeControl.Hotkeys
{
    ...
    public class HotkeyActionManager : IHotkeyActionManager
    {
        ...
        public HotkeyActionManager(params object?[] methodContainers)
        {
            ...
            GetActionMethods(methodContainers);
        }
        ...
        private void GetActionMethods(IEnumerable<object?> enumerableObjects)
        {
            foreach (var hObj in enumerableObjects)
            {
                if (hObj == null)
                    continue;
                if (hObj is IEnumerable<object> enumerable)
                    GetActionMethods(enumerable);
                else if (hObj.GetType().GetCustomAttribute<ActionAddonAttribute>() != null)

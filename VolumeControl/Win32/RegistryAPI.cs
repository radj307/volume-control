...
namespace VolumeControl.Win32
{
    public class RegistryAPI
    {
        ...
        public static RegistryKey? GetKey(RegistryKey scope, string path, bool writable = true) => scope.OpenSubKey(path, writable);
        /// <summary>
        /// Deletes the specified <see cref="RegistryKey"/> from the Windows Registry.
        /// </summary>
        /// <param name="scope">The parent registry key where the target is located.</param>
        /// <param name="path">The path from the scope key to the target.</param>
        public static void DeleteKey(RegistryKey scope, string path) => scope.DeleteSubKey(path);
        ...
        public static object? GetValue(RegistryKey scope, string path, string valueName) => GetKey(scope, path, false)?.GetValue(valueName);
        /// <summary>
        ...
        public static void SetValue(RegistryKey scope, string path, string valueName, object value) => GetKey(scope, path, true)?.SetValue(valueName, value);
        /// <summary>
        ...
        /// <param name="path">The path from the scope key to the target.</param>
        /// <param name="valueName">The name of the target value.</param>
        public static void DeleteValue(RegistryKey scope, string path, string valueName) => GetKey(scope, path, true)?.DeleteValue(valueName);
    }
}
